...
if (isset($_GET['brand'])) {
    ...
    if (count($brand) > 0) {
        foreach ($brand as $val) {
            $stbmnt = "SELECT * FROM products WHERE brand = ? AND (prevprice-price)>70;";
...
if (isset($_GET['color'])) {
    ...
    if (count($color) > 0) {
        foreach ($color as $col) {
            $stcmnt = "SELECT * FROM products WHERE color = ? AND (prevprice-price)>70;";
...
if (isset($_GET['stock'])) {
    ...
    if (count($stock) == 1) {
        if ($stock[0] == 'all') {
            $stment = "SELECT * FROM products WHERE status=1 AND (prevprice-price)>70";
            ...
            $query = "SELECT * FROM products WHERE stock > 1 AND (prevprice-price)>70;";
            ...
            $query = "SELECT * FROM products WHERE stock= 0 AND (prevprice-price)>70;";
            if ($results = $conn->query($query)) {
                while ($rowArrays = $results->fetch_assoc()) {
                    array_push($arrays, $rowArrays);
                }
...
if (isset($_GET['maxvalue']) && isset($_GET['minvalue'])) {
    ...
    $stpmnt = "SELECT * FROM products WHERE price BETWEEN ? AND ? AND (prevprice-price)>70;";
